      ...
      <section className="mx-auto max-w-3xl">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-7">
          ...
          <div className="mt-7 grid gap-4 sm:grid-cols-2">
            <Field label="Serviço">
              <select
                ...
                onChange={(event) => {
                  ...
                  setProfessionalId("");
                  setSelectedSlot("");
                  const nextService = data.services.find((item) => item.id === next);
                  if (nextService) setDurationMinutes(nextService.duration_minutes);
                }}
                className={inputClass}
              >
                {data.services.map((item) => (
                  <option key={item.id} value={item.id}>{item.name} · padrão {item.duration_minutes} min</option>
                ))}
              </select>
            </Field>
            ...
            <Field label="Profissional">
              <select
                ...
                onChange={(event) => { setProfessionalId(event.target.value); setSelectedSlot(""); }}
                className={inputClass}
              >
            ...
            <Field label="Data">
              <input
                type="date"
                min={getToday(data.establishment.timezone)}
                value={date}
                onChange={(event) => { setDate(event.target.value); setSelectedSlot(""); }}
                className={inputClass}
              />
            </Field>
            ...
            <Field label="Duração desejada">
              <select
                ...
                onChange={(event) => { setDurationMinutes(Number(event.target.value)); setSelectedSlot(""); }}
                className={inputClass}
              >
                {Array.from({ length: 16 }, (_, index) => (index + 1) * 15).map((minutes) => (
                  <option key={minutes} value={minutes}>{minutes} minutos{service?.duration_minutes === minutes ? " · padrão" : ""}</option>
                ))}
              </select>
            </Field>
          </div>
          ...
          <div className="mt-6 rounded-xl border border-border p-4">
            ...
            {!availabilityQuery.isPending && !availabilityQuery.isError && availableSlots.length === 0 ? (
              <p className="mt-4 rounded-lg bg-muted/40 p-4 text-sm text-muted-foreground">Nenhum horário disponível para esta combinação de data, profissional e duração.</p>
            ) : null}
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                {availableSlots.map((slot) => (
                  <button
                    ...
                    className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${selectedSlot === slot.startsAt ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background text-foreground hover:bg-accent"}`}
                  >
                    {slot.label}
                  </button>
                ))}
          ...
          <div className="mt-6 rounded-xl border border-border p-4">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <Field label="Nome">
                <input required minLength={2} value={customerName} onChange={(event) => setCustomerName(event.target.value)} className={inputClass} />
              </Field>
              <Field label="Telefone / WhatsApp">
                <input required minLength={8} value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} className={inputClass} />
              </Field>
          ...
          {selectedSlot ? (
            <div className="mt-6 rounded-xl bg-muted/40 p-4 text-sm">
              <p className="font-semibold text-foreground">Confira antes de confirmar</p>
              <p className="mt-1 text-muted-foreground">{service?.name} · {durationMinutes} min · {professionalId ? professionals.find((item) => item.id === professionalId)?.name : "Qualquer profissional"}</p>
              <p className="text-muted-foreground">{formatDate(selectedSlot, data.establishment.timezone)} · {formatTime(selectedSlot, data.establishment.timezone)}</p>
              <p className="mt-1 font-semibold text-foreground">{service ? formatPrice(service.price) : ""}</p>
            </div>
          ) : null}
          ...
          <button
            type="button"
            disabled={!selectedSlot || customerName.trim().length < 2 || customerPhone.trim().length < 8 || mutation.isPending}
            onClick={() => { setError(null); mutation.mutate(); }}
            className="mt-6 w-full rounded-md bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
          >
