...
import { demoBusiness } from "@/lib/scheduling/config";
...
function SchedulePage() {
  ...
  const selectedService =
    demoBusiness.services.find((service) => service.id === selectedServiceId) ??
    demoBusiness.services[0];
  const selectedProfessional =
    demoBusiness.professionals.find(
      (professional) => professional.id === selectedProfessionalId,
    ) ?? demoBusiness.professionals[0];
  const dates = Array.from({ length: 7 }, (_, index) => addDays(today, index));

  const availableSlots = useMemo(() => {
    const windows = demoBusiness.weeklySchedule[
      selectedDate.getDay() as keyof typeof demoBusiness.weeklySchedule
    ];
      ...
      <div className="mx-auto max-w-5xl space-y-8">
        <header className="space-y-2">
          ...
          <p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
            Escolha o serviço, o profissional, a data e um horário disponível. A
            agenda usa uma configuração centralizada de funcionamento, duração,
            intervalos e reservas para calcular os encaixes.
          </p>
        </header>
        ...
        <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="rounded-2xl border bg-background p-5 shadow-sm sm:p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {demoBusiness.professionals.map((professional) => (
                <Button
                  key={professional.id}
                  type="button"
                  variant={
          ...
          <div className="rounded-2xl border bg-background p-5 shadow-sm sm:p-6">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
              {dates.map((date) => {
                ...
                const isClosed =
                  !(demoBusiness.weeklySchedule[
                    date.getDay() as keyof typeof demoBusiness.weeklySchedule
                  ]?.length ?? 0);
            ...
            {availableSlots.length === 0 ? (
              <div className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">
                Não existem horários disponíveis para {selectedProfessional.name}
                nessa combinação. Tente outro profissional, serviço ou data.
              </div>
            ) : (
        ...
        <p className="text-center text-xs text-muted-foreground">
          Demonstração do fluxo público. Os dados ainda são estáticos, sem
          persistência, autenticação ou criação real de agendamentos.
        </p>
      </div>
    </main>
  );
