...
const THEMES: Record<string, BusinessTheme> = {
  ...
  "nail designer": {
    ...
    accentForeground: "oklch(0.35 0.10 12)",
    ring: "oklch(0.70 0.11 12)",
  },
  "sobrancelhas": {
  ...
  "estética": {
    ...
    accentForeground: "oklch(0.30 0.07 175)",
  ...
  "clínica": {
    ...
    accentForeground: "oklch(0.30 0.08 235)",
...
export function getBusinessTypeLabel(businessType: string | null | undefined) {
  const value = normalizeBusinessType(businessType);
  ...
  return labels[value] ?? businessType?.trim() ?? labels.outro;
}
